...
${expression ? 'Expression: "' + expression + '"\n\n' : ""}`, el);
  ...
  function src_default3(Alpine2) {
    Alpine2.directive("intersect", Alpine2.skipDuringClone((el, { value, expression, modifiers }, { evaluateLater: evaluateLater2, cleanup }) => {
      let evaluate2 = evaluateLater2(expression);
      let options = {
  ...
  // src/assets/js/components/contrast-toggle.js
  var ContrastToggle = class extends HTMLElement {
    connectedCallback() {
      this.isHighContrast = localStorage.getItem("highContrast") === "true" || localStorage.getItem("highContrast") === null && window.matchMedia("(prefers-contrast: more)").matches;
      this.button = document.createElement("button");
      this.button.type = "button";
      this.button.className = "inline-flex items-center gap-2 text-xs text-zinc-400 hc:text-zinc-900 hover:text-zinc-300 hc:hover:text-zinc-700 transition-colors cursor-pointer";
    ...
    updateState() {
      this.track.className = `relative w-7 h-4 rounded-full transition-colors ${this.isHighContrast ? "bg-primary-600" : "bg-zinc-700"}`;
      this.thumb.className = `absolute top-0.5 left-0.5 w-3 h-3 bg-white rounded-full shadow transition-transform ${this.isHighContrast ? "translate-x-3" : ""}`;
  ...
  module_default.data("accordion", (initialOpen = null) => {
    return {
  ...
  document.addEventListener(
    ...
    () => {
      ...
      console.log(
        "%c Made with love by Dupondius Games %c https://dupondius.games",
